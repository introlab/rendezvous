...
void DisplayImageBuilder::createDisplayImage(const std::vector<Image>& vcImages, const Image& outDisplayImage)
{
    ...
    if (displayDimention_ == outDisplayImage && vcCount > 0)
    {
        ...
        int firstVcLeftOffset = (outDisplayImage.width - vcCount * vcDim.width - (vcCount - 1) * VIRTUAL_CAMERA_SPACING) / 2;
...
void DisplayImageBuilder::clearVirtualCamerasOnDisplayImage(const Image& displayImage)
{
    ...
    int memsetOffset = ((displayImage.height - maxVirtualCameraDim_.height) / 2) * (displayImage.width * displayImage.bytesPerPixel);
    std::memset(displayImage.hostData + memsetOffset, BACKGROUND_VALUE, memsetSize);
}

void DisplayImageBuilder::fillImage(int offset, const Dim2<int>& vcDim, const Dim2<int>& displayDim, const RGB* vcData, RGB* displayData)
{
...
void DisplayImageBuilder::fillImage(int offset, Dim2<int> vcDim, Dim2<int> displayDim, const UYVY* vcData, UYVY* displayData)
{
...
void DisplayImageBuilder::fillImage(int offset, Dim2<int> vcDim, Dim2<int> displayDim, const YUYV* vcData, YUYV* displayData)
{
