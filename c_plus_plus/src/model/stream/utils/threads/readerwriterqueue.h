...
class ReaderWriterQueue
{
   ...
   private:
    ...
    struct ReentrantGuard
    {
        ...
        {
            assert(!inSection &&
                   "Concurrent (or re-entrant) enqueue or dequeue operation detected (only one thread at a time may "
                   "hold the producer or consumer role)");
            inSection = true;
        }
...
class BlockingReaderWriterQueue
{
   ...
   public:
    explicit BlockingReaderWriterQueue(size_t maxSize = 15) AE_NO_TSAN : inner(maxSize),
