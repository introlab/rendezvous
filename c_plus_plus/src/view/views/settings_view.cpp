      ...
      m_settings(settings)
{
    ...
    {
        m_ui->languageComboBox->addItem(Model::Transcription::languageName(
            static_cast<Model::Transcription::Language>(i)));
    }

    m_ui->outputFolderLineEdit->setText(
        m_settings
            .get(Model::General::keyName(Model::General::Key::OUTPUT_FOLDER))
            .toString());
    m_ui->languageComboBox->setCurrentIndex(
        m_settings
            .get(Model::Transcription::keyName(
                Model::Transcription::Key::LANGUAGE))
            .toInt());
    m_ui->autoTranscriptionCheckBox->setChecked(
        m_settings
            .get(Model::Transcription::keyName(
                Model::Transcription::Key::AUTOMATIC_TRANSCRIPTION))
            .toBool());

    connect(m_ui->outputFolderButton, &QAbstractButton::clicked,
            [=] { onOutputFolderButtonClicked(); });
    connect(m_ui->languageComboBox,
            qOverload<int>(&QComboBox::currentIndexChanged),
            [=](const int& index) {
                onLanguageComboboxCurrentIndexChanged(index);
            });
    connect(m_ui->autoTranscriptionCheckBox, &QCheckBox::stateChanged,
            [=](const int& state) {
                onAutoTranscriptionCheckBoxStateChanged(state);
            });
}
...
void SettingsView::onOutputFolderButtonClicked()
{
    ...
    {
        m_settings.set(
            Model::General::keyName(Model::General::Key::OUTPUT_FOLDER),
            outputFolder);
