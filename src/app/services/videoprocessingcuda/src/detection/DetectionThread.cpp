...
void DetectionThread::run()
{
    ...
    while (!isAbortRequested())
    {
        ...
        for (int i = 0; i < dewarpCount_; ++i)
        {
            ...
            {
                detections.push_back(dewarping::getAngleRectFromDewarpedImageRectangle(detection, dewarpingParams[i],
...
std::vector<DewarpingParameters> DetectionThread::getDetectionDewarpingParameters(const Dim3<int>& src, int dewarpCount)
{
    ...
    {
        dewarpingParams.push_back(dewarping::getDewarpingParameters(src, dewarpingConfig_, i * angleSpan));
    }
