#include "DewarpingHelper.h"

#include <cmath>
#include <stdexcept>

#include "utils/math/AngleCalculations.h"
#include "utils/math/GeometryUtils.h"
...
{
    DonutSlice donutSlice(static_cast<float>(imageSize.width) / 2.f, static_cast<float>(imageSize.height) / 2.f, dewarpingConfig.inRadius,
                          dewarpingConfig.outRadius, middleAngle, dewarpingConfig.angleSpan);

    return getDewarpingParameters(donutSlice, dewarpingConfig.topDistorsionFactor, dewarpingConfig.bottomDistorsionFactor);
}
...
DewarpingParameters getDewarpingParameters(DonutSlice& baseDonutSlice, float topDistorsionFactor, float bottomDistorsionFactor)
{
    ...
    return getDewarpingParametersFromNewDonutSlice(baseDonutSlice, newDonutSlice, centersDistance, bottomDistorsionFactor);
}
...
{
    AngleBox angleBox = math::convertToAngleBox(angleRect);
    DonutSlice donutSlice(fisheyeCenter.x, fisheyeCenter.y, dewarpingConfig.inRadius,
                          dewarpingConfig.outRadius, angleRect.azimuth, angleRect.azimuthSpan);
    ...
    float minElevation = math::getElevationFromImage(Point<float>(dewarpingParameters.dewarpWidth / 2.f, dewarpingParameters.dewarpHeight),
                                                     dewarpingConfig.fisheyeAngle, fisheyeCenter, dewarpingParameters);
...
Point<float> getSourcePixelFromDewarpedImage(const Point<float>& pixel, const DewarpingParameters& dewarpingParameters)
{
    ...
    float radius = pixel.y + dewarpingParameters.inRadius + dewarpingParameters.outRadiusDiff *
                                                 ...
                                                 const Dim2<int>& imageSize, const Point<float>& fisheyeCenter, float fisheyeAngle)
{
    ...
    float azimuthRight = math::getAzimuthFromImage(Point<float>(rightX, yMostRight), fisheyeCenter, dewarpingParameters);
    float elevationTop = math::getElevationFromImage(Point<float>(xMostTop, bottomY), fisheyeAngle, fisheyeCenter, dewarpingParameters);
    float elevationBottom = math::getElevationFromImage(Point<float>(xMostBottom, topY), fisheyeAngle, fisheyeCenter, dewarpingParameters);

    return math::convertToAngleRect(AngleBox(azimuthLeft, azimuthRight, elevationBottom, elevationTop));
}
...
Point<float> calculateSourcePixelPosition(const Dim2<int>& dst, const DewarpingParameters& params, int index)
{
    ...
    float factor = params.outRadiusDiff * (1 - params.bottomDistorsionFactor) *
        std::sin(math::PI * textureCoordsX) * std::sin((math::PI * heightFactor) / 2.0);
