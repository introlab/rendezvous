...
namespace
{
    ...
    const float NEW_VIRTUAL_CAMERA_CREATION_THRESHOLD = 0.5f; // Distance at which a new vc will be created instead of moved
    ...
    const float TIME_TO_GOAL_MS = 1000.f;                     // How much time is required for the camera to reach it's goal
}
...
void VirtualCameraManager::updateVirtualCameras(float elapsedTimeMs)
{
    ...
    float updateRatio = std::min(elapsedTimeMs / TIME_TO_GOAL_MS, 1.f);
    ...
    {   
        ...
        float distance = math::getApproximatedSphericalAnglesDistance(vc.elevation, azimuthDifference, elevationDifference);
...
void VirtualCameraManager::updateVirtualCamerasGoal(const std::vector<AngleRect>& goals)
{   
    if (goals.empty()) return;

    // Get a vector of non duplicate goals (duplicates are within a set threshold)
    std::vector<AngleRect> uniqueGoals = getUniqueRegions(goals);
...
std::vector<AngleRect> VirtualCameraManager::getUniqueRegions(const std::vector<AngleRect>& regions)
{
    std::vector<AngleRect> uniqueRegions;
    uniqueRegions.reserve(regions.size());

    for (const AngleRect& region : regions)
    {
        bool isGoalDuplicate = false;

        // Check if there is already a region within the angle thresholds
        for (const AngleRect& uniqueRegion : uniqueRegions)
        {
...
void VirtualCameraManager::updateRegionsBounds(std::vector<AngleRect>& regions)
{
    for (AngleRect& region : regions)
    {
        float newElevation = region.elevation + region.elevationSpan / ELEVATION_SHIFT_RATIO;
        region.elevationSpan = math::clamp(region.elevationSpan, VIRTUAL_CAMERA_MIN_ELEVATION_SPAN, srcImageMaxElevationSpan_);
...
std::multimap<float, std::pair<int, int>> VirtualCameraManager::getVcToGoalOrderedDistances(const std::vector<AngleRect>& goals)
{
    ...
    {
        const AngleRect& region = virtualCameras_[regionIndex];

        for (int goalIndex = 0; goalIndex < goalsCount; ++goalIndex)
        {
            const AngleRect& goal = goals[goalIndex];
            float distance = math::getApproximatedSphericalAnglesDistance(region.azimuth, region.elevation, goal.azimuth, goal.elevation);
