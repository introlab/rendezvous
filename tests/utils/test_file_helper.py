...
class TestFileHelper(unittest.TestCase):
    ...
    def setUp(self):
        self.testFile = os.path.join(rootPath, 'tests/utils/data.json')
        pass
    ...
    def test_writeJsonFile_not_exists(self):
        outputFile = os.path.join(rootPath, 'tests/utils/test.json')
        jsonData = {
            'test' : 'rendezvous'
        }
    ...
    def test_readJsonFile_not_exists(self):
        readFile = os.path.join(rootPath, 'tests/utils/notExists.json')
    ...
    def test_readJsonFile_exists(self):
        readFile = os.path.join(rootPath, 'tests/utils/test.json')
        data = {
            'test2' :  42
        }
    ...
    def test_readJsonFile_not_json(self):
        readFile = os.path.join(rootPath, 'tests/utils/test.txt')
        data = 123423545435
        FileHelper.writeJsonFile(readFile, data)
        try: 
            FileHelper.readJsonFile(readFile)
